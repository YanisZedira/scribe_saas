...
function MeetingReport({ meeting }) {
  ...
  return <>
    ...
    {tab === "overview" && <div className="report-overview"><article className="executive-card"><span className="card-label">En bref</span><h2>{report.executive_summary}</h2><div className="report-facts"><span><strong>{report.decisions.length}</strong> décisions</span><span><strong>{report.actions.length}</strong> actions</span><span><strong>{report.open_questions.length}</strong> questions ouvertes</span></div></article>{report.quality && <article className="quality-card"><div><strong>{report.quality.coverage_percent}%</strong><span>transcript couvert</span></div><div><strong>{report.quality.source_linked_facts_percent}%</strong><span>faits sourcés</span></div><div><strong>{report.quality.identified_speakers_percent}%</strong><span>voix identifiées</span></div></article>}<PodcastPlayer script={report.podcast_script} meetingId={meeting.id} overview={report.podcast_overview}/><article className="content-card minutes-card"><span className="card-label">Compte rendu détaillé</span><p>{report.detailed_minutes}</p></article><InsightList title="Points clés" items={report.key_points} render={(item) => <><strong>{item.topic}</strong><p>{item.detail}</p></>}/><InsightList title="Risques et vigilances" items={report.risks} render={(item) => <><strong>{item.risk}</strong><p>{item.mitigation || "Aucune réponse définie pendant la réunion."}</p></>}/></div>}
