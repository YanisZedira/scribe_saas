...
export function RemoteMeetingView({ id, onBack }) {
  ...
  async function stopNow() {
    if (!confirm("Arrêter Scribe et effacer immédiatement le transcript en cours ?")) return;
  ...
  return <section className="page remote-page">
    ...
    <header className="remote-header">
      <div><div className="remote-meta"><span className={`status ${meeting.status}`}>{statusLabels[meeting.status]}</span><span className={`platform-chip ${meeting.platform}`}>{meeting.platform === "google_meet" ? "Google Meet" : "Microsoft Teams"}</span></div><h1>{meeting.title}</h1><p>{formatDate(meeting.created_at)} · {formatDuration(meeting.duration_seconds)}</p></div>
    ...
    {meeting.status === "completed" && <MeetingReport meeting={meeting}/>}
...
function MeetingReport({ meeting }) {
  ...
  return <>
    ...
    {tab === "overview" && <div className="report-overview"><article className="executive-card"><span className="card-label">En bref</span><h2>{report.executive_summary}</h2><div className="report-facts"><span><strong>{report.decisions.length}</strong> décisions</span><span><strong>{report.actions.length}</strong> actions</span><span><strong>{report.open_questions.length}</strong> questions ouvertes</span></div></article><PodcastPlayer script={report.podcast_script} meetingId={meeting.id} overview={report.podcast_overview}/><article className="content-card minutes-card"><span className="card-label">Compte rendu détaillé</span><p>{report.detailed_minutes}</p></article><InsightList title="Points clés" items={report.key_points} render={(item) => <><strong>{item.topic}</strong><p>{item.detail}</p></>}/><InsightList title="Risques et vigilances" items={report.risks} render={(item) => <><strong>{item.risk}</strong><p>{item.mitigation || "Aucune réponse définie pendant la réunion."}</p></>}/></div>}
    ...
    {tab === "actions" && <div className="action-table"><div className="action-table-head"><span>Action</span><span>Responsable</span><span>Échéance</span><span>Priorité</span></div>{report.actions.length ? report.actions.map((item, index) => <div className="action-table-row" key={index}><strong>{item.task}</strong><span>{item.owner ? speakerNames[item.owner] || item.owner : "À confirmer"}</span><span>{item.due_date || "Non définie"}</span><span className={`priority-label ${item.priority || "medium"}`}>{priorityLabel(item.priority)}</span></div>) : <EmptySection text="Aucune action explicite n’a été formulée."/>}</div>}
...
function formatDate(value) { return new Date(value).toLocaleString("fr-FR", { dateStyle: "long", timeStyle: "short" }); }
